...
public class EncToolComplete {
	...
	private static void decryptRSA() {	
		try{
			...
			//open and read ciphertext file.
			System.out.println("Openning file to read: "+inFile);
			RandomAccessFile rawDataFromFile = new RandomAccessFile(inFile, "r");
			byte[] encAESkey = new byte[128];
			byte[] cipherText = new byte[((int)rawDataFromFile.length())-128];
			System.out.println("Reading Data");
			rawDataFromFile.read(encAESkey);
			rawDataFromFile.read(cipherText);
			System.out.println("Enc Key: "+byteArrayToHexString(encAESkey));
			System.out.println("Cipher: "+byteArrayToHexString(cipherText));

			// Load the RSA key from the keystore
			KeyStore myKeyStore = KeyStore.getInstance("JKS");
			FileInputStream inStream = new FileInputStream(keyStore);
			myKeyStore.load(inStream, password);
			PrivateKey privatekey = (PrivateKey) myKeyStore.getKey("mykey", password); 

			// Initiate the RSA Cipher Object
			Cipher rsaCipher = Cipher.getInstance("RSA");
			rsaCipher.init(Cipher.DECRYPT_MODE, privatekey);

			// Decrypt the AES Key
			byte[] aesKeyBytes = rsaCipher.doFinal(encAESkey);
			System.out.println("Key: "+byteArrayToHexString(aesKeyBytes));
			...
			byte[] plainText = decAEScipher.doFinal(cipherText);
	...
	private static void decryptAESCCM() {
		try{
			...
			//Set up the AES key & cipher object in CCM mode using BC as provider
			SecretKeySpec secretKeySpec = new SecretKeySpec(hexStringToByteArray(hexKey), "AES");
			Security.insertProviderAt(new BouncyCastleProvider(), 1);
			Cipher encAESCCMcipher = Cipher.getInstance("AES/CCM/NoPadding","BC");			
			IvParameterSpec ivSpec = new IvParameterSpec(iv);
			encAESCCMcipher.init(Cipher.DECRYPT_MODE, secretKeySpec,ivSpec);

			//decrypt the data
			byte[] plainText = encAESCCMcipher.doFinal(cipherText);
